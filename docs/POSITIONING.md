...
| **OCSF 1.1.0** | ✅ shipped for File / Network / Process Activity | `--event-format=ocsf` on `aegisbpfd run` emits OCSF JSON for `BlockEvent` (File Activity 1001), `NetBlockEvent` (Network Activity 4001), `ExecEvent` (Process Activity 1007 — Launch / Allowed / Informational), and `ForensicEvent` (Process Activity 1007 — Launch / Denied + Blocked at the exec gate). `KernelBlockEvent` and `OverlayCopyUpEvent` stay Aegis-native because OCSF 1.1 has no analog class. See `docs/SIEM_INTEGRATION.md` §OCSF and `src/ocsf_formatter.cpp`. |
