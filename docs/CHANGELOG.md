...
### Added — Operator Contract
- **Explicit ringbuf overflow policy** (`--ringbuf-overflow-policy=...`, env `AEGIS_RINGBUF_OVERFLOW_POLICY`, `src/ringbuf_policy.{hpp,cpp}`, `tests/test_ringbuf_policy.cpp`, `docs/EVENT_LOSS_AND_BACKPRESSURE.md`) — closes the operator-visibility half of `docs/POSITIONING.md` §4.1 #5 ("Ring-buffer backpressure policy is implicit"). The dual-path priority/main + shed-telemetry-first behaviour that has shipped since the priority ringbuf landed is now a named, parser-validated, startup-logged policy (`priority-fallback`) rather than an implicit hard-coded behaviour. Selectable via the new `--ringbuf-overflow-policy=priority-fallback` CLI flag (or the equivalent `AEGIS_RINGBUF_OVERFLOW_POLICY` env var, useful for systemd unit overrides). Parsing is case-insensitive and accepts the underscore alias `priority_fallback`. Reserved roadmap names (`sample`, `spool`, `spool-to-disk` and underscore variants) are recognised by the parser as a distinct `Reserved` error class so the daemon refuses to start with a "reserved for a future release" log line instead of silently falling back to the default — a typo or premature config-management rollout is loud, not silent. The active policy and its description are emitted at startup as a structured INFO log line (`Ringbuf overflow policy {policy=priority-fallback, description=...}`) so journal pipelines can grep-assert the contract did not change between releases. Two new ctest CLI rejection tests (`cli_run_rejects_unknown_ringbuf_overflow_policy`, `cli_run_rejects_reserved_ringbuf_overflow_policy`) pin both error paths via `WILL_FAIL=TRUE`. New 8-test GTest unit suite (`RingbufOverflowPolicy.*`) covers default-on-empty, kebab-case, underscore alias, case-insensitive variants (`PRIORITY-FALLBACK`, `Priority-Fallback`), reserved-name distinction from unknown-name, garbage rejection (`not-a-policy`, `priority`, `fallback`, `priority-fallback-extra`), name canonicalisation stability, and description hygiene (non-empty, no trailing period since it composes into a larger log line). The `daemon_run()` signature gains a defaulted `RingbufOverflowPolicy ringbuf_overflow_policy = RingbufOverflowPolicy::PriorityFallback` parameter so existing callers and embedders don't have to be updated. No BPF-side change: the policy is declarative on top of the existing implementation today; future releases that introduce `sample` or `spool-to-disk` will require an explicit opt-in via this same flag, so a default-mode change cannot ship silently.
