...
- **Post-init capability drop** (`--drop-caps`, `src/capabilities.{hpp,cpp}`) — opt-in reduction of the daemon's Linux capability set to the minimum required for runtime operation. After all initialization is complete (BPF load + map setup + LSM hook attach + ringbuf creation + `/proc` reconciliation) and after Landlock + seccomp are applied, the daemon drops its effective and permitted capability sets to `{CAP_BPF, CAP_PERFMON, CAP_DAC_READ_SEARCH, CAP_SYS_RESOURCE}`. Bounding set is reduced to the same retain mask via `PR_CAPBSET_DROP` (done first, while `CAP_SETPCAP` is still in the effective set); ambient and inheritable sets are cleared. Implementation uses raw `capget(2)` / `capset(2)` syscalls and direct `prctl(PR_CAPBSET_DROP, ...)` with no libcap or libcap-ng dependency. On kernel < 5.8 (lacks `CAP_BPF` / `CAP_PERFMON`), logs an INFO message and returns success without dropping; the daemon continues with full root caps. The drop happens before any worker thread (heartbeat, etc.) is spawned so that workers inherit the reduced set via the per-thread credential copy at `clone()` time. New 7-test GTest suite (`tests/test_capabilities.cpp`) verifies default config shape, kernel split-support probe, snapshot reading, empty-retain rejection, effective-set reduction, idempotence, and bounding-set reduction (root-only tests skip via `GTEST_SKIP()` when not running as root). Closes the "Daemon hardening: split capabilities" row in the Standards Alignment matrix and the corresponding limitation in `README.md` §"Honest Limitations" #8.
