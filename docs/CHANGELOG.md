...
### Added — Operator Multi-Cluster
- **`aegisfleet` read-only fleet aggregator** (`operator/cmd/aegisfleet/main.go`, `operator/internal/fleet/`) — small CLI that connects to many clusters via per-cluster kubeconfigs and renders a unified table or JSON view of every `AegisPolicy` and `AegisClusterPolicy` it can see. Closes the "no fleet view across clusters yet" gap from Honest Limitations #4 without introducing a new control plane: each spoke keeps running its own `aegis-operator`, and `aegisfleet` only reads. Inputs: `--kubeconfig-dir=<dir>` (cluster name = filename minus extension) and/or repeated `--kubeconfig=<name=path>` / `--kubeconfig=<path>` flags; duplicate names error early. Output: tabwriter table (default) with columns `CLUSTER NAMESPACE NAME SCOPE MODE PHASE READY GEN NODES AGE`, or stable JSON `{rows, errors}` via `--output=json`. Per-cluster errors (kubeconfig load, connect, RBAC, CRD missing) never abort aggregation — they're collected into the `FLEET ERRORS` section so a single broken cluster doesn't hide the rest of the fleet. Exit codes are CI-friendly: `0` healthy, `2` at least one policy reports `Ready=False`, `3` at least one cluster failed (cluster failure outranks policy failure so infrastructure issues surface first). Per-cluster `--timeout` (default 15s) caps the cost of a slow cluster. Ready-condition message is surfaced into a per-row `error` field so non-Ready policies show *why*. Pure aggregation logic is split into `internal/fleet/{fleet,discovery,render}.go` with a `ClientFactory` indirection so unit tests use `client/fake` clients keyed off `rest.Config.Host`; the CLI under `cmd/aegisfleet` is a thin flag-parsing shell. New 11-test Go suite (`internal/fleet/{fleet,discovery}_test.go`) covers cross-cluster aggregation+sorting, per-cluster error scoping, namespace filter semantics (namespaced only, cluster-scoped unaffected), exit-code contract, table column presence, and JSON round-trip. Out of scope: policy fan-out / hub `AegisFleetPolicy` CRD / cross-cluster admission control. RBAC needs only `get`+`list` on `aegispolicies`/`aegisclusterpolicies`. New `make build-fleet` target. Documented in `docs/MULTI_CLUSTER.md`. Updates Honest Limitation #4 from "no fleet view across clusters yet" to "fleet view shipped; cross-cluster fan-out still on roadmap".
