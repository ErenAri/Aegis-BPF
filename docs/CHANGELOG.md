...
### Added — Tooling
- **`aegisbpf simulate` command** (`src/commands_simulate.{hpp,cpp}`) — replays a JSONL stream of past AegisBPF block events through a candidate policy and reports what would change, with no kernel side effects. Reads events from `--events <file|->` (one JSON object per line, `#` comments + blank lines tolerated), parses the candidate policy via `--policy <path>` (or the applied policy at `kPolicyAppliedPath` if absent), and reports four buckets: `events_parsed` (block-class events seen), `matched` (deny-rule match under candidate), `newly_blocked` (matched AND original action was AUDIT — the "what would enforce break" answer), `policy_drift` (was non-AUDIT in input but candidate has no matching deny — likely-removed rule). Allowlist precedence (`allow_cgroup`) short-circuits deny matches. `--strict` exits 1 if `newly_blocked > 0`, suitable for CI gates that require zero new breaks before promoting a policy. `--json` emits a structured report including up to `--sample-limit=N` (default 5) sample events per bucket. Eight GTest cases (`tests/test_simulate.cpp`) cover required-args validation, the four-bucket counting on a mixed input stream, allow-cgroup short-circuit, parse-error tolerance, strict-mode exit codes, and sample-limit clamping. Closes the "policy simulation / dry-run diffing" item in `docs/POSITIONING.md` §4.2 #10.
