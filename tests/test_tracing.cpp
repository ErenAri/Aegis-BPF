...
Result<KernelFeatures> test_detect_full()
{
    ...
    features.ima = true;
    features.ima_appraisal = false;
    features.bpf_ima_helpers = true;
    features.landlock = true;
    features.landlock_abi = 4;
    features.ipe = true;
    features.fs_verity = true;
    features.bpf_token = true;
    features.bpf_arena = true;
    features.user_ringbuf = true;
    features.sched_ext = true;
    features.open_coded_iterators = true;
    features.bpf_xattr_kfuncs = true;
    features.bpf_send_signal_task = true;
    features.binary_auth = true;
    features.lsm_list = "lockdown,capability,landlock,yama,ipe,bpf,integrity";
    features.kernel_version = "6.13.0";
    features.kernel_major = 6;
    features.kernel_minor = 13;
...
TEST(TracingTest, DaemonRunGuardsSigkillBehindBuildAndRuntimeFlags)
{
    ...
    logger().set_json_format(true);
    {
        int rc = daemon_run(false, false, false, false, 0, kEnforceSignalKill, false, LsmHookMode::FileOpen, 0, 1,
...
TEST(TracingTest, DaemonRunRejectsSilentPartialAttachContract)
{
    ...
    logger().set_json_format(true);
    {
        ...
        int rc = daemon_run(false, false, false, false, 0, kEnforceSignalTerm, false, LsmHookMode::Both, 0, 1,
                            kSigkillEscalationThresholdDefault, kSigkillEscalationWindowSecondsDefault);
        EXPECT_EQ(rc, 1);
    }
    logger().set_output(&std::cerr);
...
TEST(TracingTest, DaemonRunWritesCapabilityReportArtifact)
{
...

    {
        ...
        DaemonHookGuard hooks(test_config_ok, test_detect_full, test_memlock_ok, test_load_bpf_ok,
                              test_ensure_layout_ok, test_set_agent_config_ok, test_populate_survival_ok,
                              test_setup_agent_cgroup_ok, test_attach_all_full_contract_no_network_hooks);
        int rc = daemon_run(true, false, false, false, 0, kEnforceSignalTerm, false, LsmHookMode::FileOpen, 0, 1,
                            kSigkillEscalationThresholdDefault, kSigkillEscalationWindowSecondsDefault);
        EXPECT_EQ(rc, 1);
    }
    ...
    EXPECT_NE(payload.find("\"future_lsm\""), std::string::npos);
    EXPECT_NE(payload.find("\"active_lsms\""), std::string::npos);
    EXPECT_NE(payload.find("\"landlock_abi\": 4"), std::string::npos);
    EXPECT_NE(payload.find("\"ipe\": true"), std::string::npos);
    EXPECT_NE(payload.find("\"fs_verity\": true"), std::string::npos);
    EXPECT_NE(payload.find("\"bpf_token\": true"), std::string::npos);
    EXPECT_NE(payload.find("\"next_gen_bpf\""), std::string::npos);
    EXPECT_NE(payload.find("\"arena\": true"), std::string::npos);
    EXPECT_NE(payload.find("\"user_ringbuf\": true"), std::string::npos);
    EXPECT_NE(payload.find("\"sched_ext\": true"), std::string::npos);
    EXPECT_NE(payload.find("\"open_coded_iterators\": true"), std::string::npos);
    EXPECT_NE(payload.find("\"xattr_kfuncs\": true"), std::string::npos);
    EXPECT_NE(payload.find("\"bpf_send_signal_task\": true"), std::string::npos);
    EXPECT_NE(payload.find("\"binary_auth\": true"), std::string::npos);
