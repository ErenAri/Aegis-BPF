                                       ...
                                       const CapabilityReportRuntimeState& runtime_state)
{
    ...
    return atomic_write_stream(output_path, [&](std::ostream& out) -> bool {
        ...
        out << "  \"future_lsm\": {\n";
        out << "    \"active_lsms\": \"" << json_escape(features.lsm_list) << "\",\n";
        out << "    \"landlock\": " << (features.landlock ? "true" : "false") << ",\n";
        out << "    \"landlock_abi\": " << static_cast<int64_t>(features.landlock_abi) << ",\n";
        out << "    \"ipe\": " << (features.ipe ? "true" : "false") << ",\n";
        out << "    \"fs_verity\": " << (features.fs_verity ? "true" : "false") << ",\n";
        out << "    \"bpf_token\": " << (features.bpf_token ? "true" : "false") << "\n";
        out << "  },\n";
        out << "  \"next_gen_bpf\": {\n";
        out << "    \"arena\": " << (features.bpf_arena ? "true" : "false") << ",\n";
        out << "    \"user_ringbuf\": " << (features.user_ringbuf ? "true" : "false") << ",\n";
        out << "    \"sched_ext\": " << (features.sched_ext ? "true" : "false") << ",\n";
        out << "    \"open_coded_iterators\": " << (features.open_coded_iterators ? "true" : "false") << ",\n";
        out << "    \"xattr_kfuncs\": " << (features.bpf_xattr_kfuncs ? "true" : "false") << ",\n";
        out << "    \"bpf_send_signal_task\": " << (features.bpf_send_signal_task ? "true" : "false") << ",\n";
        out << "    \"binary_auth\": " << (features.binary_auth ? "true" : "false") << "\n";
        out << "  },\n";
        out << "  \"features\": {\n";
        ...
        out << "    \"fs_verity\": " << (features.fs_verity ? "true" : "false") << ",\n";
        out << "    \"bpf_token\": " << (features.bpf_token ? "true" : "false") << ",\n";
        out << "    \"bpf_arena\": " << (features.bpf_arena ? "true" : "false") << ",\n";
        out << "    \"user_ringbuf\": " << (features.user_ringbuf ? "true" : "false") << ",\n";
        out << "    \"sched_ext\": " << (features.sched_ext ? "true" : "false") << ",\n";
        out << "    \"open_coded_iterators\": " << (features.open_coded_iterators ? "true" : "false") << ",\n";
        out << "    \"bpf_xattr_kfuncs\": " << (features.bpf_xattr_kfuncs ? "true" : "false") << ",\n";
        out << "    \"bpf_send_signal_task\": " << (features.bpf_send_signal_task ? "true" : "false") << ",\n";
        out << "    \"binary_auth\": " << (features.binary_auth ? "true" : "false") << ",\n";
